...
async function fetchConReintentos(url, maxRetries = 3) {
...
async function sincronizarJornales(supabase) {
  try {
    ...
    for(let i = 0; i < jornales.length; i += BATCH_SIZE){
      ...
      try {
        const { data, error } = await supabase.from('jornales').upsert(batch, {
...
async function sincronizarIRPF(supabase) {
  try {
    ...
    for (const values of rows){
      ...
      try {
        ...
        const { error } = await supabase.from('configuracion_usuario').upsert({
          chapa: chapa.trim(),
...
async function sincronizarPrimas(supabase) {
  try {
    ...
    for (const values of rows){
      ...
      try {
        // Usar upsert para actualizar o insertar
        const { error } = await supabase.from('primas_personalizadas').upsert({
          chapa: chapa.trim(),
          fecha: fechaISO,
...
async function sincronizarForo(supabase) {
  try {
    ...
    for(let i = 0; i < mensajes.length; i += BATCH_SIZE){
      ...
      try {
        const { data, error } = await supabase.from('mensajes_foro').upsert(batch, {
